...
def move_slider(s1, s2, target_points: list, current_point_attr_name, step=1):
    ...
    if target_points[1] < current_position_2:
        offset_2 = int((current_position_2 - target_points[1]) / step)
        slider_2.send_keys(Keys.ARROW_LEFT * offset_2)
    else:
        offset_2 = int((target_points[1] - current_position_2) / step)
        slider_2.send_keys(Keys.ARROW_RIGHT * offset_2)
...
move_slider(S1_LOCATOR, S2_LOCATOR, target_points=[200, 700], current_point_attr_name="aria-valuenow", step=5)
