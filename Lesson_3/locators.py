# 1 Main Title (What is your learning goal?)
MAIN_TITLE = ("xpath", "//h1")

# 2 Sign In Button
SIGN_IN_BUTTON = ("xpath", "//button[@class='btn btn-outline-light btn-sm text-nowrap']")

# 3 Start for Free
START_FOR_FREE_BUTTON = ("xpath", "//button[@class='btn btn-primary btn-sm text-nowrap']")

# 4 HyperSkill
HYPERSKILL = ("xpath", "//div[@class='text-logo d-none d-xl-inline-block ml-xl-2']")

# 5 Explore
EXPLORE = ("xpath", "//a[@class='nav-link dropdown-toggle tw-flex tw-items-center']")

# 6 Pricing
PRICING = ("xpath", "//li[@click-event-target='pricing']")

# 7 For Business
FOR_BUSINESS = ("xpath", "//li[@click-event-target='join_as_organization']")

# 8 Explore all tracks
EXPLORE_ALL_TRACKS_TEXT = ("xpath", "//h2[text()='Explore all tracks']")

# 9 Find the right one by choosing a specific area of knowledge
FIND_THE_RIGHT_TEXT = ("xpath", "//p[text()='Find the right one by choosing a specific area of knowledge']")

# 10 All tracks Button
ALL_TRACKS_BUTTON = ("xpath", "(//a[@click-event-part='main'])[1]")

# 11 Top tracks Button
TOP_TRACKS_BUTTON = ("xpath", "(//a[@click-event-part='main'])[2]")

# 12 Beginner-friendly Button
BEGINNER_FRIENDLY_BUTTON = ("xpath", "(//a[@click-event-part='main'])[3]")

# 13 Python Button
PYTHON_BUTTON = ("xpath", "(//a[@click-event-part='main'])[5]")

# 14 Java Button
JAVA_BUTTON = ("xpath", "(//a[@click-event-part='main'])[6]")

# 15 Javascript Button
JAVASCRIPT_BUTTON = ("xpath", "(//a[@click-event-part='main'])[7]")

# 16 Kotlin Button
KOTLIN_BUTTON = ("xpath", "(//a[@click-event-part='main'])[8]")

# 17 Go Button
GO_BUTTON = ("xpath", "(//a[@click-event-part='main'])[9]")

# 18 Android  Button
ANDROID_BUTTON = ("xpath", "(//a[@click-event-part='main'])[10]")

# 19 C++  Button
C_PLUS_PLUS_BUTTON = ("xpath", "(//a[@click-event-part='main'])[11]")

# 20 Generative AI Button
GENERATIVE_AI_BUTTON = ("xpath", "(//a[@click-event-part='main'])[12]")

# 21 Math Button
MATH_BUTTON = ("xpath", "(//a[@click-event-part='main'])[13]")

# 22 Frontend  Button
FRONTEND_BUTTON = ("xpath", "(//a[@click-event-part='main'])[14]")

# 23 SQL and Databases Button
SQL_DATABASES_BUTTON = ("xpath", "(//a[@click-event-part='main'])[15]")

# 24 Python Core Card
PYTHON_CORE_CARD = ("xpath", "(//div[@class='card-body'])[1]")

# 25 Reddit Icon
REDDIT_ICON = ("xpath", "//a[@click-event-target='Reddit']")

# 26 Facebook Icon
FACEBOOK_ICON = ("xpath", "//a[@click-event-target='Facebook']")

# 27 Linkedin Icon
LINKEDIN_ICON = ("xpath", "//a[@click-event-target='Linkedin']")

# 28 Discord Icon
DISCORD_ICON = ("xpath", "//a[@click-event-target='Discord']")

# 29 Instagram Icon
INSTAGRAM_ICON = ("xpath", "//a[@click-event-target='Instagram']")

# 30 TikTok Icon
TIKTOK_ICON = ("xpath", "//a[@click-event-target='TikTok']")

# 31 YouTube Icon
YOUTUBE_ICON = ("xpath", "//a[@click-event-target='YouTube']")
